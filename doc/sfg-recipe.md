...
1. Generate Sealed Secrets resources required by Sterling File Gateway.
    ...
    1. Generate a Sealed Secret for the DB2 credentials.
        ```bash
        B2B_DB_USER=dbadmin B2B_DB_SECRET=password ./b2b-db-secret-secret.sh
        ```
...
2. Generate Helm Chart values.yaml for the Sterling File Gateway Helm Chart:
    ```
